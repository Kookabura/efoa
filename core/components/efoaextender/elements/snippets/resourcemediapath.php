 ...
 * &forTemplates (optional) number id templates for create folder
 */
...
$fortemplates = $modx->getOption('forTemplates', $scriptProperties, '');
...
if ($resource = $modx->getObject('modResource', $docid)) {
    ...
    $path = str_replace('{alias}', str_replace('-','.',$resource->get('alias')), $path);
    ...
    if ($templatename = $resource->getOne('Template')) {
        $path = str_replace('{templatename}', $templatename->get('templatename'), $path);
    }
    if ($user = $modx->user) {
	...
	if($fortemplates !== ''){
		$fortemplates = array_map('trim',explode(',',$fortemplates));
		if(!in_array($template, $fortemplates)){
			$createpath = false;
		}
	}
